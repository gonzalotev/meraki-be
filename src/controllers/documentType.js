...
class DocumentTypeController {
    static async fetch(req, res, next) {
        try {
            ...
            const documentsTypes = await DocumentTypeService.fetch({ page, search: searchValue });
            const total = await DocumentTypeService.getTotal({ search: searchValue });
            res.send({ documentsTypes, total });
        } catch (error) {
            next(error);
        }
    }
